...
class BitacoraComentarios(ctk.CTkFrame):
    ...
    def _compute_indicators(self):
        ...
        with get_db_connection() as conn:
            cur = conn.cursor(); cur.execute("SELECT fecha_parto FROM partos WHERE animal_id=? ORDER BY fecha_parto DESC LIMIT 5", (self.animal_id,))
            partos = cur.fetchall()
        self.ind_births_hist.set(" | ".join([(r['fecha_parto'] if isinstance(r, sqlite3.Row) else r[0]) for r in partos]) if partos else "-")
        with get_db_connection() as conn:
            cur = conn.cursor(); cur.execute("""
