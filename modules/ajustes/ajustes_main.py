...
class AjustesFrame(ctk.CTkFrame):
    ...
    def crear_widgets(self):
        ...
        # ============ PLANTILLAS DE CARGA ============
        # Sección separada con manejo robusto de errores
        self._crear_seccion_plantillas(scroll_container)

        # Sección de Licencia (opcional, si no carga continúa)
        try:
            ...
            self.logger.warning(f"Licencia no disponible: {e}")
            # Continuar sin la sección de licencia
    ...
    def _crear_seccion_plantillas(self, parent):
        """Crea la sección de plantillas de carga de forma robusta."""
        try:
            plantillas_frame = ctk.CTkFrame(parent, corner_radius=10)
            ...
            ctk.CTkButton(
                gen_all_row, 
                text="⚡ Generar Todas las Plantillas (23)", 
            ...
            ctk.CTkLabel(
                gen_all_row,
                text="Crea 23 plantillas Excel listas para importar",
                font=("Segoe UI", 10),
                text_color="gray"
            ).pack(side="left", padx=10)
            
            # Fila de descarga individual
            sel_row = ctk.CTkFrame(plantillas_frame, fg_color="transparent")
            sel_row.pack(fill="x", padx=10, pady=(5, 15))
            
            ctk.CTkLabel(sel_row, text="O descargue una plantilla específica:", font=("Segoe UI", 12)).pack(anchor="w", pady=(0, 5))
            
            combo_btn_row = ctk.CTkFrame(sel_row, fg_color="transparent")
            combo_btn_row.pack(fill="x")
            
            # Combo con plantillas
            try:
                template_names = get_template_names()
                if not template_names:
                    template_names = ["Sin plantillas disponibles"]
            except Exception as e:
                self.logger.warning(f"Error obteniendo plantillas: {e}")
                template_names = ["Error al cargar plantillas"]
            
            self.plantilla_combo = ctk.CTkComboBox(
                combo_btn_row, 
            ...
            if template_names:
                self.plantilla_combo.set(template_names[0])
            ...
            self.logger.error(f"Error creando sección plantillas: {e}")
            # Fallback: mostrar error
            error_frame = ctk.CTkFrame(parent, corner_radius=10)
            error_frame.pack(fill="x", pady=10)
            ctk.CTkLabel(error_frame, text="📦 Plantillas de carga (Excel)", font=("Segoe UI", 16, "bold")).pack(anchor="w", padx=10, pady=(10, 5))
            ctk.CTkLabel(
                error_frame, 
    ...
    def _load_preferences(self):
        ...
        try:
            val = str(prefs.get("auto_switch_bitacora", "true")).strip().lower()
            self.auto_switch_var.set("Sí" if val in ("1","true","sí","si","on","yes") else "No")
        except Exception:
            pass
