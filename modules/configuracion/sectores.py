...
class SectoresFrame(ctk.CTkFrame):
    ...
    def crear_widgets(self):
        ...
        # Frame del formulario (más grande y con layout mejorado)
        form_frame = ctk.CTkFrame(scroll_container, corner_radius=10)
        form_frame.pack(pady=10, padx=5, fill="both", expand=False)

        ctk.CTkLabel(form_frame, text="📝 Nuevo Sector", font=("Segoe UI", 16, "bold")).pack(anchor="w", pady=10, padx=15)
        ...
        # Finca (ancho completo)
        ctk.CTkLabel(grid_frame, text="Finca *:", anchor="w").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.combo_finca = ctk.CTkComboBox(grid_frame, height=35)
        self.combo_finca.set(PLACEHOLDERS.get("finca", "Seleccione una finca"))
        self.combo_finca.grid(row=0, column=1, columnspan=3, sticky="ew", padx=5, pady=5)

        # Código y Nombre (lado a lado, expandidos)
        ctk.CTkLabel(grid_frame, text="Código *:", anchor="w").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        self.entry_codigo = ctk.CTkEntry(grid_frame, height=35)
        self.entry_codigo.grid(row=1, column=1, sticky="ew", padx=5, pady=5)
        
        ctk.CTkLabel(grid_frame, text="Nombre *:", anchor="w").grid(row=1, column=2, sticky="w", padx=5, pady=5)
        self.entry_nombre = ctk.CTkEntry(grid_frame, height=35)
        self.entry_nombre.grid(row=1, column=3, sticky="ew", padx=5, pady=5)

        # Comentario (ancho completo)
        ctk.CTkLabel(grid_frame, text="Comentario:", anchor="nw").grid(row=2, column=0, sticky="nw", padx=5, pady=5)
        self.text_comentario = ctk.CTkTextbox(grid_frame, height=80)
        self.text_comentario.grid(row=2, column=1, columnspan=3, sticky="nsew", padx=5, pady=5)
        grid_frame.rowconfigure(2, weight=1)

        # Botones formulario
        btn_frame = ctk.CTkFrame(form_frame, fg_color="transparent")
        btn_frame.pack(fill="x", padx=15, pady=15)
        ctk.CTkButton(btn_frame, text="💾 Guardar Sector", command=self.guardar_sector, height=40,
                      fg_color="green", hover_color="#006400", font=("Segoe UI", 13, "bold")).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="🔄 Limpiar", command=self.limpiar_formulario, height=40, font=("Segoe UI", 12)).pack(side="left", padx=5)

        # Separador
        ctk.CTkLabel(scroll_container, text="📋 Sectores Registrados", font=("Segoe UI", 16, "bold")).pack(anchor="w", pady=(20, 5), padx=5)

        # Búsqueda rápida (arriba de la tabla)
        filtro_frame = ctk.CTkFrame(scroll_container, fg_color="transparent")
        filtro_frame.pack(fill="x", padx=5, pady=(0, 10))
        ctk.CTkLabel(filtro_frame, text="🔍 Buscar:").pack(side="left", padx=5)
        self.entry_buscar = ctk.CTkEntry(filtro_frame, placeholder_text=PLACEHOLDERS.get("busqueda_general", "Buscar"), height=35)
        self.entry_buscar.pack(side="left", fill="x", expand=True, padx=5)
        self.entry_buscar.bind("<KeyRelease>", lambda e: self.filtrar_tabla())

        # Tabla de sectores (más grande)
        ...
        self.tabla = ttk.Treeview(table_frame, columns=("id", "finca", "codigo", "nombre", "comentario"),
                                  show="headings", displaycolumns=("finca", "codigo", "nombre", "comentario"), height=18)
        column_config = [
            ("id", "ID", 40),
            ("finca", "Finca", 180),
            ("codigo", "Código", 120),
        ...
        ctk.CTkButton(action_frame, text="✏️ Editar Seleccionado", command=self.editar_sector, height=40, font=("Segoe UI", 12)).pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="🗑️ Eliminar Seleccionado", command=self.eliminar_sector, height=40,
                      fg_color="red", hover_color="#8B0000", font=("Segoe UI", 12)).pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="📥 Importar Excel", command=self.importar_excel, height=40, font=("Segoe UI", 12)).pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="🔄 Actualizar Lista", command=self.cargar_sectores, height=40, font=("Segoe UI", 12)).pack(side="left", padx=5)
