...
class CausaMuerteFrame(ctk.CTkFrame):
    ...
    def crear_widgets(self):
        # Título
        titulo = ctk.CTkLabel(self, text="💀 Configuración de Causas de Muerte", font=("Segoe UI", 20, "bold"))
        titulo.pack(pady=10)
        ...
        ctk.CTkLabel(row2, text="Tipo Causa:", width=100).pack(side="left", padx=5)
        self.combo_tipo = ctk.CTkComboBox(row2, 
            values=["Enfermedad", "Accidente", "Natural", "Sacrificio", "Otros"],
            width=150)
        ...
        ctk.CTkButton(btn_frame, text="💾 Guardar Causa", command=self.guardar_causa, 
        ...
        ctk.CTkButton(action_frame, text="✏️ Editar Seleccionado", command=self.editar_causa).pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="🗑️ Eliminar Seleccionado", command=self.eliminar_causa, 
                     fg_color="red", hover_color="#8B0000").pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="🔄 Actualizar Lista", command=self.cargar_causas).pack(side="left", padx=5)
        ctk.CTkButton(action_frame, text="📥 Importar Excel", command=self.importar_excel).pack(side="left", padx=5)

    def guardar_causa(self):
        """Guarda una nueva causa de muerte"""
        ...
        try:
            ...
            messagebox.showerror("Error", "Ya existe una causa con ese código.")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar la causa:\n{e}")

    def cargar_causas(self):
        ...
        try:
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT codigo, descripcion, tipo_causa, comentario FROM causa_muerte WHERE estado = 'Activo'")
    ...
    def eliminar_causa(self):
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar la causa '{codigo}'?"):
            try:
                with db.get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("UPDATE causa_muerte SET estado = 'Inactivo' WHERE codigo = ?", (codigo,))
                    conn.commit()
                messagebox.showinfo("Éxito", "Causa eliminada.")
                self.cargar_causas()
            except Exception as e:
    ...
    def importar_excel(self):
        """Importar causas de muerte desde Excel.
        Se esperan encabezados: codigo,descripcion,tipo_causa,comentario,estado
        """
                ...
                for idx, fila in enumerate(filas, start=2):
                    ...
                    try:
                        ...
                        if cursor.fetchone()[0] > 0:
                            errores.append(f"Fila {idx}: causa con código '{codigo}' ya existe")
                            continue

                        cursor.execute("INSERT INTO causa_muerte (codigo, descripcion, tipo_causa, comentario, estado) VALUES (?, ?, ?, ?, ?)", (
                            codigo,
                            descripcion,
                            str(fila.get('tipo_causa') or "Enfermedad").strip(),
                            str(fila.get('comentario') or "").strip() or None,
                            str(fila.get('estado') or "Activo").strip()
                        ))
                        importados += 1
                    except sqlite3.IntegrityError:
                        errores.append(f"Fila {idx}: causa duplicada")
                    except Exception as e:
