...
class DestinoVentaFrame(ctk.CTkFrame):
    ...
    def crear_widgets(self):
        # Título
        titulo = ctk.CTkLabel(self, text="🏷️ Configuración de Destinos de Venta", font=("Segoe UI", 20, "bold"))
        titulo.pack(pady=10)
        ...
        # Separador
        ctk.CTkLabel(self, text="📋 Destinos de Venta Registrados", font=("Segoe UI", 16, "bold")).pack(anchor="w", pady=(20,5), padx=20)
        ...
        # Tabla
        self.tabla = ttk.Treeview(table_frame, columns=("codigo", "descripcion", "tipo_destino", "comentario"), show="headings", height=12)
        
        column_config = [
            ("codigo", "Código", 120),
            ("descripcion", "Descripción", 200),
            ("tipo_destino", "Tipo Destino", 120),
    ...
    def guardar_destino(self):
        """Guarda un nuevo destino de venta"""
        ...
        try:
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO destino_venta (codigo, descripcion, tipo_destino, comentario, estado)
                    VALUES (?, ?, ?, ?, ?)
                """, (
                    codigo,
                    descripcion,
    ...
    def cargar_destinos(self):
        ...
        try:
            with db.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT codigo, descripcion, tipo_destino, comentario FROM destino_venta WHERE estado = 'Activo'")
                ...
                for idx, fila in enumerate(filas, start=2):
                    ...
                    try:
                        ...
                        """, (
                            codigo,
                            descripcion,
                            str(fila.get('tipo_destino') or "Local").strip(),
