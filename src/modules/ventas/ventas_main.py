...
class VentasModule(ctk.CTkFrame):
    def __init__(self, master):
        ...
        # Colores del módulo
        self.color_bg, self.color_hover = obtener_colores('ventas')
    ...
    def crear_widgets(self):
        ...
        titulo = ctk.CTkLabel(header, text="💰 Gestión de Ventas", font=("Segoe UI", 22, "bold"), text_color="white")
    ...
    def crear_formulario_venta(self):
        ...
        ctk.CTkLabel(row0, text="Finca *:", width=150).pack(side="left", padx=5)
        self.combo_finca = ctk.CTkComboBox(row0, width=300, command=self._on_finca_change)
    ...
    def cargar_combos(self):
        ...
        try:
            with get_db_connection() as conn:
                ...
                cursor.execute("""
                    SELECT id, nombre FROM finca ORDER BY nombre
                """)
                fincas = [f"{row[0]}|{row[1]}" for row in cursor.fetchall()]
                self.combo_finca.configure(values=fincas)
                
                # Cargar todos los animales por defecto
                self._cargar_animales_por_finca(None)
                
                self.logger.info(f"Cargadas {len(fincas)} fincas")

        except Exception as e:
            self.logger.error(f"Error al cargar combos: {e}")
            messagebox.showerror("Error", f"No se pudieron cargar los datos: {e}")

    def _cargar_animales_por_finca(self, finca_id):
        """Carga los animales según la finca seleccionada"""
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()

                if finca_id:
                    # Animales activos de la finca seleccionada
                    cursor.execute("""
                ...
                self.logger.info(f"Cargados {len(animales)} animales")

        except Exception as e:
            self.logger.error(f"Error al cargar animales: {e}")
            messagebox.showerror("Error", f"No se pudieron cargar los animales: {e}")

    def _on_finca_change(self, value):
        """Callback cuando cambia la finca seleccionada"""
        try:
            if value and "|" in value:
                finca_id = int(value.split("|")[0])
                self._cargar_animales_por_finca(finca_id)
            else:
                self._cargar_animales_por_finca(None)
        except Exception as e:
            self.logger.error(f"Error al cambiar finca: {e}")
    ...
    def validar_datos_venta(self):
        ...
        if not self.combo_finca.get() or self.combo_finca.get() == "Seleccione la finca":
            errores.append("Seleccione una finca")
